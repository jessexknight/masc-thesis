...
\begin{itemize}
  ...
  and use it as the test case ($N_r$ = $N-1$; $N_e = 1$);
  repeat $N$ times.
  ...
  and use it as the test set ($N_r$ = $N-B$; $N_e = B$);
...
The remainder of this chapter explores model variants which yield performance improvements.
For sake of comparison, results from a minimal working algorithm are first presented.
...
\begin{figure}
  ...
  \begin{subfigure}{\plotwidth}
    \centering\includegraphics[height=7cm]{objective-surf}
    \caption{$\J(\bb)$ surface}%
    \label{fig:obj-surf}
  \end{subfigure}
  \begin{subfigure}{\plotwidth}
    \centering\includegraphics[height=7cm]{objective-LR}
