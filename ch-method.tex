...
% [x] Overlapping distributions
% [ ] Bias field
% [x] DAWM
% [x] Partial volume effect
% [-] Artifacts
% [-] Image variability
% [ ] (registration)
% [ ] (standardization)
...
%\cite{Rueckert1999}
%N3/4 \citeauthor{Tustison2010} \cite{Tustison2010}
%SPM New Segment \citeauthor{Ashburner2005} \cite{Ashburner2005}
%FSL FLIRT \citeauthor{Jenkinson2002} \cite{Jenkinson2002}
%THERE ARE SO MANY \citeauthor{Klein2009} \cite{Klein2009}...
%ans SPM is definitely not the best...
%BUT: SPM is one of only a few which can do multi-modal image registration, which is helpful, since 
%still: a FLAIR template does exist \cite{Winkler2012}
\footnote{Full disclosure: this topic was not investigated, except for visual inspection of the MNI-space images produced by SPM. Since these appeared acceptable, no other tools were explored.}
...
\end{equation}
where $G_{\sigma}$ is a Gaussian convolution kernel with width $\sigma$.
