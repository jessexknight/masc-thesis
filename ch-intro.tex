...
\begin{table}
  ...
  \centering{\setlength{\tabcolsep}{2pt}
    \begin{tabular}{ccrclcrclcrclcl}\toprule
      Tissue && \multicolumn{3}{c}{T1 (ms)} && \multicolumn{3}{c}{T2 (ms)} && \multicolumn{3}{c}{$K [H]$ (a.u.)}&& Ref\\\midrule
...
\begin{enumerate}[itemsep=0pt,topsep=0pt]
  ...
  \textbf{Overlapping distributions:}\\
    Using the relaxation constants in Table~\ref{tab:t1t2tissues}
    in conjunction with the FLAIR signal equation (\ref{eq:MRI-IR}),
    the predicted intensity distributions of GM and WMH can overlap,
    depending on the choice of TE, TR, and TI
    (cf. Table~\ref{tab:database} for typical values and \S~\ref{s:simflair} for modelling).
    As a result, the intensity of image voxels alone
    cannot be used to determine their class~\cite{Mortazavi2012}.
  ...
  \textbf{Partial volume effect:}\\
    ...
    can be modelled as a linear mixture of the components~\cite{Santago1995}.
...
from the content of an observed image to an image of labels or classes -- in this case, tissues.
...
\par
Features used for segmentation can be derived from
...
since it is the only image-specific information (i.e.\ the evidence).
...
of the 53 works reviewed (Table~\ref{tab:wmlsegtable}),
less than half use more than one scanner for validation, and only 4 use more than three.
...
Graylevel thresholding techniques~\cite{Jack2001,Smart2011,Samaille2012,Schmidt2012,Khademi2014}
...
(mean SI $\le 0.60$) than other works;
...
for data from new scanners, as will be discussed in \S~\ref{s:CVframeworks}.
...
This thesis therefore aims to produce a WMH segmentation algorithm
...
Chapter 2 motivates and develops the voxel-wise logistic regression model,
including expected challenges and solutions with this approach;
Chapter 3 explores optimization of model components through experiment,
and then presents segmentation performance results under various cross validation schemes;
Chapter 4 discusses the performance results,
